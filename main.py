from waiter import Agent, Table, ClientGroup, Kitchen, AgentControllerRL, Order
import numpy as np
import random
...
def main_game(n_tables, n_groups, mode="constant"): 
    ...
    diff_Q = 0
    n_batches = 40 # How many "groups of client-groups" we will let in during the entire process
    ...
    while current_batch < n_batches:
                ...
                for group in groups:
                    if group.state != 4:
                        ...
                        wait_ls[group.batch] += 1 #Track the total waiting time of each batch
